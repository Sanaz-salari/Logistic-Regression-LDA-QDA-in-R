...
honor <- read.table(file = "honor.csv", header = T, sep = ",")
head(honor)
str(honor)
...
# Let's build this model: logit(p) = log(p/(1-p))= beta0 + bet1*math + beta2*female + beta3*read
...
# Acknowledgement: the above tutorial is adopted (with modification) from https://stats.idre.ucla.edu/other/mult-pkg/faq/general/faq-how-do-i-interpret-odds-ratios-in-logistic-regression/
...
m5 <- glm(hon ~ math + read, data = honor, family = binomial)
summary(m5)
# log(p/1-p) = Beta0 + Beta1*math + Beta2*read
#log(p/1-p)=0.85949
#p/1-p= exp(0.85949)
#p = exp(0.85949)/(1+exp(0.85949)) = 0.7025541

# E2. There are two students, A and B. A's math score is 10 points higher than B's. Build an appropriate model to answer: The odds of A getting in the honors class is _____ times more than the odds of B getting in the honors class. 
# A. 2.775
# B. 3.775
# C. 4.775
...
m8 <- glm(hon ~ math, data = honor, family = binomial)
summary(m8)
